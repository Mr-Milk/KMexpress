kmplot_OS<-function(test_final,km_type){
require(survival)
data<-test_final
...
#single gene and single transcript
	{
		...
		dat$Q11high<-as.numeric(as.character(dat$Expression));
...
filename=paste(Expression,"_Overall_survival",".pdf",sep="")
pdf(file =filename,onefile=TRUE,paper = "special", height = 8, width = 8)
#change height and width according to number of figures
a<-coxph(Surv(as.numeric(as.character(dat$Time)), dat$Outcome) ~ dat$Q11high) 
coeffs <- coef(summary(a))
temp<-as.matrix(coeffs)#temp,col1 coef;col2 exp(coef);col3 se(coef);col4 z;col5 pr(>|z|)
#p[i,1]=temp[5]
#HR[i,1]=temp[2]
fit<-survfit(Surv(as.numeric(as.character(dat$Time)),dat$Outcome)~dat$Q11high)
par(mar = c(6,1,1,0),oma = c(1,1,1,1))
plot(fit,col=c("red","green"),xlab="Time(months)",ylab="Overall Survival",lwd=4)
...
#text(pos=4,quantile(as.numeric(as.character(dat$Time)),0.05),0.5,paste("logrank p=",as.character(p1), sep=""))
#text(pos=4,quantile(as.numeric(as.character(dat$Time)),0.05),0.4,paste("Hazard ratio=",as.character(HR1), sep=""))
#text(pos=4,quantile(as.numeric(as.character(dat$Time)),0.05),0.3,paste(Expression,km_type, sep="_"))
remove(a)
dev.off()
filename=paste(Expression,"_Overall_survival",".png",sep="")
...
p1=round(temp[5],4)#4 is number of digitals
HR1=round(temp[2],4)
#text(quantile(as.numeric(as.character(dat$Time)),0.75),1,paste("logrank p=",as.character(p1), sep=""))
#text(quantile(as.numeric(as.character(dat$Time)),0.75),0.9,paste("Hazard ratio=",as.character(HR1), sep=""))
#text(quantile(as.numeric(as.character(dat$Time)),0.75),0.8,paste(Expression,km_type, sep="_"))
mtext(paste("logrank p=",as.character(p1), sep=""), side = 1, line = -2.4, adj = 1, cex = 1.2, outer = TRUE)
mtext(paste("Hazard ratio=",as.character(HR1), sep=""), side = 1, line = -1.2, adj = 1, cex = 1.2, outer = TRUE)
mtext(paste(Expression,km_type, sep="_"), side = 1, line = 0, adj = 1, cex = 1.2, outer = TRUE)

#text(pos=4,quantile(as.numeric(as.character(dat$Time)),0.05),1,paste("logrank p=",as.character(p1), sep=""))
#text(pos=4,quantile(as.numeric(as.character(dat$Time)),0.05),0.4,paste("Hazard ratio=",as.character(HR1), sep=""))
#text(pos=4,quantile(as.numeric(as.character(dat$Time)),0.05),0.3,paste(Expression,km_type, sep="_"))
remove(a)
dev.off()
