Boxplot_clinical_without_survival_revised<-function(Samples,GeneSymbol,exp,Dataset)
{
...
if (dim(test_exp1)[1]==1){
test_exp<-as.numeric(as.character(test_exp1[1,]))
test_exp<-log2(test_exp+1)
}

if (dim(test_exp1)[1]>1){
xdim<-dim(test_ex)[1];
test_exp<-as.numeric(as.character(test_exp1[1:xdim,]))
test_exp<-log2(test_exp+1)
}
...
for (m in 1:length(labels_factor))
  for (t in 1:length(labels_factor))
   {pvalue_temp<-wilcox.test(temp1[phenotype==labels_factor[m],1],temp1[phenotype==labels_factor[t],1])
    pvalue[m,t]<-round(pvalue_temp$p.value,4)
    fold_temp<-(mean(temp1[phenotype==labels_factor[m],1])+1)/(mean(temp1[phenotype==labels_factor[t],1])+1)
